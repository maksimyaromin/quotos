...
export function QuotaGlyph({ size = 15 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" style={{ flex: "0 0 auto" }}>
      ...
      <path d="M4.46 12.02a5.4 5.4 0 1 1 7.08 0" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" />
    </svg>
  );
}
...
export function MenuBarTile({ pins = [], onClick, showStrip = true, style }) {
  ...
  const tile = (
    <button type="button" onClick={onClick} style={{
      ...
      height: 22, padding: "0 var(--space-1-5)", border: 0, borderRadius: "var(--radius-xs)",
      background: "transparent", cursor: "pointer",
      color: showStrip ? "rgba(255,255,255,0.92)" : "var(--text-primary)",
      fontFamily: "var(--font-mono)", fontSize: "12px", fontWeight: "var(--weight-medium)",
  ...
  return (
    <div style={{
      ...
      <span style={{ display: "inline-flex", alignItems: "center", gap: 3, color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-sans)", fontSize: 12 }}>
        100%
      ...
      <span style={{ color: "rgba(255,255,255,0.82)", fontFamily: "var(--font-sans)", fontSize: 12 }}>Mon 9:41</span>
    </div>
  );
}
