...
// This directory is a verbatim copy of docs/design/system/ (the design
// system's source of truth), and no build step syncs them — the standing
// rule is "when you change a component, edit both copies identically". This
// test is that rule, executed: every file here (tests excepted — they live
// only on the app side) must exist in docs/design/system/ at the same
// relative path with identical bytes. docs/design/system/ owning extra
// material the app doesn't consume (cards, ui_kits, the prebuilt bundle) is
// fine and not checked.

const appCopyDir = path.dirname(fileURLToPath(import.meta.url));
const sourceDir = path.resolve(appCopyDir, "../../docs/design/system");
...
describe("design-system copies stay in sync", () => {
  ...
  it("every app-copy file is byte-identical to its docs/design/system counterpart", () => {
    const missing = [];
    const diverged = [];
    ...
    expect(missing, "app-copy files with no docs/design/system counterpart — add them there too").toEqual([]);
